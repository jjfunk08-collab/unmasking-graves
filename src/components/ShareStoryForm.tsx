...
export function ShareStoryForm() {
  ...
  if (status === "success") {
    return (
      <div
        role="status"
        className="rounded-3xl border border-green/40 bg-green/5 p-8 text-center md:p-12"
      >
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-green text-navy-900">
      ...
      <div className="rounded-2xl border border-navy/10 p-5">
        <label className="flex items-start gap-3 text-sm leading-relaxed text-ink/85">
          <input
            ...
            className="mt-1 h-5 w-5 shrink-0 rounded border-navy/30 text-green focus:ring-green"
          />
          <span>
...
const inputClass =
  "w-full rounded-xl border border-navy/20 bg-white px-4 py-3 text-base text-ink placeholder:text-muted/60 focus:border-navy focus:outline-none focus:ring-2 focus:ring-green/40";
